...
//  PI is defined in  myNM.h 옮기기 여길로
#define PI 3.14159265368979323846264338327950288412

// Problem1: Single equation of 1st order ODE  숙제에 정의한 함수 그대로 쓰기  나는 Func
double odeFunc_rc(const double t, const double v);

// Problem2: Single equation of 2nd order ODE   이거는 넘어가기
void odeFunc_mck(const double t, const double Y[], double dYdt[]);


// Single Equation : odeEM, odeEU   숙제는 odeEU랑 odeEM 이었음 !
// y0 = initial condition
// initial condition을 함수 내에서 정의하기 보다는 Parameter로 정의해버리는게 좋음
...
int main(int argc, char* argv[])
{
	...
	unsigned int N = (b - a) / h + 1;       // Number of datas
	double y_EU[200]  = { 0 };				// Cannot use y_EU[N]
	double y_EM[200]  = { 0 };              // 큰 무리가 없는 값 이내에서 over해서 array 크기를 임의로 크게 주기 !
	double y_RK2[200] = { 0 };
	double y_RK4[200] = { 0 };
